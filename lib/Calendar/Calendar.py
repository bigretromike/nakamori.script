...
if str(nt.addon.getSetting("calendar_adv")) == "True":
    image_row_span = int(nt.addon.getSetting("calendar_rows_span"))
    image_col_span = int(nt.addon.getSetting("calendar_cols_span"))
else:
    image_row_span = 8
    image_col_span = 4
max_row_count = (image_row_span * int(nt.addon.getSetting("calendar_rows"))) + 2
max_col_count = (image_col_span * int(nt.addon.getSetting("calendar_cols")))
...
class Calendar(pyxbmct.BlankDialogWindow):
    ...
    def draw(self):
        bg = pyxbmct.Image(os.path.join(img, 'calendar', 'black_dot.png'), aspectRatio=0)
        ...
        for sers in _data['series']:
            if page_count_ < _start_page:
                ...
                if air_date in used_dates:
                    ...
                    your_label = your_label.strftime(nt.addon.getSetting("calendar_format"))
                    ...
                    image0 = pyxbmct.Image(os.path.join(img, 'calendar', day_images[day_count]), aspectRatio=2)
...
class Wizard(pyxbmct.AddonDialogWindow):
    def __init__(self, title):
        ...
        self.label_login = pyxbmct.Label(label=nt.addon.getLocalizedString(30076), textColor='0xFFFFFFFF')
        self.label_pass = pyxbmct.Label(label=nt.addon.getLocalizedString(30077), textColor='0xFFFFFFFF')
        # edit
    ...
    def _test_net(self):
        if nt.get_server_status(ip=str(self.edit_ip.getText()), port=str(self.edit_port.getText())):
            ...
            nt.addon.setSetting(id="port", value=port)
            self.button_test_net.setLabel(label='OK', textColor='0xff7aad5c', focusedColor='0xff7aad5c')
        else:
            self.button_test_net.setLabel(label=nt.addon.getLocalizedString(30072),
    ...
    def _test_pass(self):
        ...
        if nt.valid_user():
            ...
            self.button_test_pass.setLabel(label=nt.addon.getLocalizedString(30072),
                                           textColor='0xFFDF1818', focusedColor='0xFFDF1818')
    ...
    def set_active_controls(self):
        ...
        self.edit_ip.setText(nt.addon.getSetting("ipaddress"))
        self.edit_port.setText(nt.addon.getSetting("port"))
        self.edit_login.setText(nt.addon.getSetting("login"))
        self.edit_pass.setText(nt.addon.getSetting("password"))
